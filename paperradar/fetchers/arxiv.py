...
def fetch(max_results=100):
    ...
    for q in SEARCH_TERMS:
        url = f"https://export.arxiv.org/api/query?search_query=all:{urllib.parse.quote(q)}&start=0&max_results={max_results}&sortBy=submittedDate&sortOrder=descending"
        try:
        ...
        for entry in root.findall("a:entry", NS):
            ...
            if not english_only(blob):      continue
            if not pass_hard_filters(blob): continue
            link = ""
            for l in entry.findall("a:link", NS):
                if l.get("type") == "text/html":
                    link = l.get("href"); break
            published = sanitize_text(entry.findtext("a:published", default="", namespaces=NS))
            authors = [sanitize_text(a.findtext("a:name", default="", namespaces=NS)) for a in entry.findall("a:author", NS)]
            ...
            items.append(Item(
                ...
                title=title, abstract=summary, url=link or sanitize_text(entry.findtext("a:id", default="", namespaces=NS)),
                published=published, source="arxiv", authors=[x for x in authors if x], venue=journal_ref,
